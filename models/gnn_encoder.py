...
class GNNLayer(nn.Module):
    """
    可配置的图神经网络层，实现了门控图卷积网络层。
    使用线性变换来更新节点特征（U、V）和边特征（A、B、C），然后通过ReLU激活函数进行非线性变换。
    支持多种聚合操作（sum、mean、max），并可以选择是否使用边门控机制。
    forward方法更新节点和边特征，aggregate方法执行特征聚合操作。
  """
...
class PositionEmbeddingSine(nn.Module):  # 位置嵌入
    """
  This is a more standard version of the position embedding, very similar to the one
  used by the Attention is all you need paper, generalized to work on images.
  类似于“Attention is all you need”论文中的位置嵌入，但针对图像进行了泛化
  """
...
class GNNEncoder(nn.Module):
  ...
  """
    ...
    def __init__(self, n_layers, hidden_dim, out_channels=1, aggregation="sum", norm="layer",
        ...
        aggregation: 聚合方法，默认为"sum"（可以是"sum"、"mean"或"max"）。
        norm: 归一化方法，默认为"layer"（可以是"layer"或"batch"）。
        learn_norm: 是否学习归一化参数。
        track_norm: 是否跟踪归一化统计信息。
        gated: 是否使用边门控机制。
        sparse: 是否使用稀疏张量。
        use_activation_checkpoint: 是否使用激活检查点。
        node_feature_only: 是否仅使用节点特征。
        """
